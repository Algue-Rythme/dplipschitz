...
cfg.learning_rate = 2.5e-2
cfg.num_gradient_steps = 7000
cfg.num_epochs = -1
cfg.batch_size = 256
...
class LipschitzCNN(nn.Module):
  ...
  num_classes: int
  scale = cfg.net_scale
  hidden_widths: Sequence[int] = (1, 1, 2)
...
def create_train_state(rng, num_classes):
  ...
  dummy_batch = jnp.zeros([cfg.batch_size, 28, 28, 1])
...
def get_datasets():
  ...
  train_ds = tfds.load('mnist', split='train', shuffle_files=True, data_dir=data_dir)
  test_ds = tfds.load('mnist', split='test', shuffle_files=False, data_dir=data_dir)
